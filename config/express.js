    ...
    User.findOne({uid: profile.id}, function(err, user) {
      if (user) {
        ...
        if (profile.emails && profile.emails[0] && profile.emails[0].value) {
          ...
          if (domain !== 'andela.co') {
            err = "You can only use an 'andela.co' email address";
            return done(err, null)
          };
          if (account === 'bernard.ojengwa') {
            data.is_staff = true;
          };
        };
        if (profile.photos && profile.photos[0] && profile.photos[0].value) {
          photo = profile.photos[0].value;
          data.photo = photo.split('?')[0];
        };
...
module.exports.http = {
  ...
  customMiddleware: function(app) {
    ...
    passport.use(new GoogleStrategy({
      clientID: '910968958898-u1k3m7jliol507vum6u3oli3cvt5qph2.apps.googleusercontent.com',
      clientSecret: 'pQnmH5SNPGJn6-1hbFH84bkq',
